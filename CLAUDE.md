...
2. **Explanations never contradict the engine.** The classifier detects motifs; Stockfish produces ground-truth PV and eval; Claude only paraphrases and teaches. Any explanation that contradicts the engine's best line is a bug — fix it, don't ship it.
...
4. **Locally runnable end-to-end.** `make serve` must boot the full product. The only external dependency is the Claude API for the explanation layer.
...
- **Claude API** (`claude-opus-4-6` primary, `claude-sonnet-4-6` fallback for cheap moves) for explanations, with prompt caching on the system prompt.
